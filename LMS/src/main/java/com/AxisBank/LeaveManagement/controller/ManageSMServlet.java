...
import com.AxisBank.LeaveManagement.util.ManageSMDaoRepoUtil;
...
@WebServlet("/manageSeniorManager")
public class ManageSMServlet extends HttpServlet{
		...
		try {
			if(status.equalsIgnoreCase("Approved"))
			{
				if(s.equalsIgnoreCase("Sick Leave")||s.equalsIgnoreCase("Casual Leave")||s.equalsIgnoreCase("Personal Leave")||s.equalsIgnoreCase("Marriage Leave"))
	            {
	            	...
	            	{
	            		if(ab<=15){
	            			ManageLeavesSM a=new ManageLeavesSM(id, name, leaveType, fromDate, toDate, ab,
	            					leaveReason,balance, status);
	            			ManageSMDaoRepo h=ManageSMDaoRepoUtil.getObject();
