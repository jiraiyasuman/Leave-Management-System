...
public class SmLoginDao implements SmLoginDaoRepo{
private static Connection conn=null;
private static SmLogin h=null;
public SmLoginDao(Connection conn)
{
	this.conn=conn;
}
public boolean insert(SmLogin h)throws Exception
{
	boolean f=false;
	try {
		String sql="INSERT INTO `leavemanagementsystem`.`loginsm` (`Name`, `Email`, `Password`) VALUES (?, ?, ?);";
...
{
	try {
		String sql="SELECT * FROM leavemanagementsystem.loginsm WHERE Email=? AND Password=?";
...
public boolean update(String user,String password)
{
	boolean f=false;
	try {
		String sql="UPDATE leavemanagementsystem.loginsm SET Password='"+password+"' WHERE Email='"+user+"'";
